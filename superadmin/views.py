...
@method_decorator(login_required(login_url='login'), name='dispatch')
class addlocalpackage(TemplateView):
    template_name = "superadmin/add_localpackage.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        blist = Brand.objects.filter(status='active')
        models = Model.objects.all()
        rp = RidePricing.objects.all()
        context = {'catlist': list(catlist), 'blist': list(blist), 'models': models, 'rp': list(rp)}
        return context

    def post(self, request):
        city = request.POST['city']
        model_id = request.POST['car_category']
        package_name = request.POST['package_name']
        price_type = request.POST['price_type']
        company_commission = Decimal(request.POST.get('company_commission', '0') or '0')
        min_amount_ac = Decimal(request.POST.get('min_amount_ac', '0') or '0')
        min_amount_non_ac = Decimal(request.POST.get('min_amount_non_ac', '0') or '0')
        max_km_per_day_ac = Decimal(request.POST.get('max_km_per_day_ac', '0') or '0')
        max_km_per_day_non_ac = Decimal(request.POST.get('max_km_per_day_non_ac', '0') or '0')
        extra_hr_charge_ac = Decimal(request.POST.get('extra_hr_charge_ac', '0') or '0')
        extra_hr_charge_non_ac = Decimal(request.POST.get('extra_hr_charge_non_ac', '0') or '0')
        extra_km_charge_ac = Decimal(request.POST.get('extra_km_charge_ac', '0') or '0')
        extra_km_charge_non_ac = Decimal(request.POST.get('extra_km_charge_non_ac', '0') or '0')
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage', '0') or '0')
        gst_tax_percentage = Decimal(request.POST.get('gst_tax_percentage', '0') or '0')
        driver_allowance = Decimal(request.POST.get('driver_allowance', '0') or '0')
        additional_info = request.POST.get('additional_info', '').strip()
        driver_per_km_commission_type = request.POST['driver_per_km_commission_type']
        driver_per_km_commission_charge = Decimal(request.POST.get('driver_per_km_commission_charge', '0') or '0')
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge', '0') or '0')

        # Create and save AirportPricing instance
        localpackage = RidePricing(
            city=city,
            price_type=price_type,
            car_category=Model.objects.get(model_id=model_id),
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class LocalPackageListView(ListView):
    model = RidePricing
    template_name = "superadmin/view_localpackage.html"
    
    # def get_queryset(self):
    #     return RidePricing.objects.filter(price_type='localpackage') 

@method_decorator(login_required(login_url='login'), name='dispatch')
class DeleteLocalPackage(View):
    def  get(self, request):
        
        ride_pricing_id = request.GET.get('ride_pricing_id', None)
        RidePricing.objects.get(ride_pricing_id=ride_pricing_id).delete()
        data = {
            'deleted': True
            }
        return JsonResponse(data)
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class EditLocalPackage(TemplateView):
    template_name = 'superadmin/edit_localpackage.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            context['ride_pricing_id'] = self.kwargs['id']
            rp = RidePricing.objects.filter(ride_pricing_id=context['ride_pricing_id'])
        except:
            rp = RidePricing.objects.filter(ride_pricing_id=context['ride_pricing_id'])
            
        context = {'catlist':list(catlist),'rp':list(rp)}
        return context

@method_decorator(login_required(login_url='login'), name='dispatch')
class UpdateLocalPackage(APIView):
    def post(self, request):
        # Update local package data
        ride_pricing_id = request.POST.get('ride_pricing_id')
        price_type = request.POST.get('price_type')
        city = request.POST.get('city')
        car_category = Model.objects.get(model_id=request.POST.get('car_category'))
        package_name = request.POST.get('package_name')
        company_commission = Decimal(request.POST.get('company_commission'))
        min_amount_ac = Decimal(request.POST.get('min_amount_ac'))
        min_amount_non_ac = Decimal(request.POST.get('min_amount_non_ac'))
        max_km_per_day_ac = Decimal(request.POST.get('max_km_per_day_ac'))
        max_km_per_day_non_ac = Decimal(request.POST.get('max_km_per_day_non_ac'))
        extra_hr_charge_ac = Decimal(request.POST.get('extra_hr_charge_ac'))
        extra_hr_charge_non_ac = Decimal(request.POST.get('extra_hr_charge_non_ac'))
        extra_km_charge_ac = Decimal(request.POST.get('extra_km_charge_ac'))
        extra_km_charge_non_ac = Decimal(request.POST.get('extra_km_charge_non_ac'))
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage'))
        gst_tax_percentage = Decimal(request.POST.get('gst_tax_percentage'))
        driver_allowance = Decimal(request.POST.get('driver_allowance'))
        additional_info = request.POST.get('additional_info')
        driver_per_km_commission_type = request.POST.get('driver_per_km_commission_type')
        driver_per_km_commission_charge = Decimal(request.POST.get('driver_per_km_commission_charge'))
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge'))


        local = RidePricing.objects.get(ride_pricing_id=ride_pricing_id)
        local.price_type=price_type,
        local.city=city,
        local.car_category=car_category,
        local.package_name=package_name,
        local.company_commission=company_commission,
        local.min_amount_ac=min_amount_ac,
        local.min_amount_non_ac=min_amount_non_ac,
        local.max_km_per_day_ac=max_km_per_day_ac,
        local.max_km_per_day_non_ac=max_km_per_day_non_ac,
        local.extra_hr_charge_ac=extra_hr_charge_ac,
        local.extra_hr_charge_non_ac=extra_hr_charge_non_ac,
        local.extra_km_charge_ac=extra_km_charge_ac,
        local.extra_km_charge_non_ac=extra_km_charge_non_ac,
        local.driver_commission_percentage=driver_commission_percentage,
        local.gst_tax_percentage=gst_tax_percentage,
        local.driver_allowance=driver_allowance,
        local.additional_info=additional_info,
        local.driver_per_km_commission_type=driver_per_km_commission_type,
        local.driver_per_km_commission_charge=driver_per_km_commission_charge,
        local.dead_per_km_charge=dead_per_km_charge,
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class AddLocalPrice(TemplateView):
    template_name = "superadmin/add_localprice.html"
    ...
    def post(self, request):
        price_type = request.POST['price_type']
        city = request.POST['city']
        model_id = request.POST['car_category']  # Assuming car_category is a ForeignKey
        company_commission = Decimal(request.POST.get('company_commission', '0') or '0')
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage', '0') or '0')
        vat_tax_percentage = Decimal(request.POST.get('vat_tax_percentage', '0') or '0')
        waiting_time_per_min_charge = Decimal(request.POST.get('waiting_time_per_min_charge', '0') or '0')
        driver_per_km_commission_type = request.POST.get('driver_per_km_commission_type', '').strip()
        driver_per_km_commission_charge = Decimal(request.POST.get('driver_per_km_commission_charge', '0') or '0')
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge', '0') or '0')


        # Day Time Ride Non AC
        customer_day_base_fare_non_ac = Decimal(request.POST.get('customer_day_base_fare_non_ac', '0') or '0')
        customer_day_per_min_charge_non_ac = Decimal(request.POST.get('customer_day_per_min_charge_non_ac', '0') or '0')
        customer_day_price_0_5_km_non_ac = Decimal(request.POST.get('customer_day_price_0_5_km_non_ac', '0') or '0')
        customer_day_price_5_10_km_non_ac = Decimal(request.POST.get('customer_day_price_5_10_km_non_ac', '0') or '0')
        customer_day_price_10_15_km_non_ac = Decimal(request.POST.get('customer_day_price_10_15_km_non_ac', '0') or '0')
        customer_day_price_above_15_km_non_ac = Decimal(request.POST.get('customer_day_price_above_15_km_non_ac', '0') or '0')

        driver_day_base_fare_non_ac = Decimal(request.POST.get('driver_day_base_fare_non_ac', '0') or '0')
        driver_day_per_min_charge_non_ac = Decimal(request.POST.get('driver_day_per_min_charge_non_ac', '0') or '0')
        driver_day_price_0_5_km_non_ac = Decimal(request.POST.get('driver_day_price_0_5_km_non_ac', '0') or '0')
        driver_day_price_5_10_km_non_ac = Decimal(request.POST.get('driver_day_price_5_10_km_non_ac', '0') or '0')
        driver_day_price_10_15_km_non_ac = Decimal(request.POST.get('driver_day_price_10_15_km_non_ac', '0') or '0')
        driver_day_price_above_15_km_non_ac = Decimal(request.POST.get('driver_day_price_above_15_km_non_ac', '0') or '0')

        # Night Time Ride Non AC
        customer_night_base_fare_non_ac = Decimal(request.POST.get('customer_night_base_fare_non_ac', '0') or '0')
        customer_night_per_min_charge_non_ac = Decimal(request.POST.get('customer_night_per_min_charge_non_ac', '0') or '0')
        customer_night_price_0_5_km_non_ac = Decimal(request.POST.get('customer_night_price_0_5_km_non_ac', '0') or '0')
        customer_night_price_5_10_km_non_ac = Decimal(request.POST.get('customer_night_price_5_10_km_non_ac', '0') or '0')
        customer_night_price_10_15_km_non_ac = Decimal(request.POST.get('customer_night_price_10_15_km_non_ac', '0') or '0')
        customer_night_price_above_15_km_non_ac = Decimal(request.POST.get('customer_night_price_above_15_km_non_ac', '0') or '0')

        driver_night_base_fare_non_ac = Decimal(request.POST.get('driver_night_base_fare_non_ac', '0') or '0')
        driver_night_per_min_charge_non_ac = Decimal(request.POST.get('driver_night_per_min_charge_non_ac', '0') or '0')
        driver_night_price_0_5_km_non_ac = Decimal(request.POST.get('driver_night_price_0_5_km_non_ac', '0') or '0')
        driver_night_price_5_10_km_non_ac = Decimal(request.POST.get('driver_night_price_5_10_km_non_ac', '0') or '0')
        driver_night_price_10_15_km_non_ac = Decimal(request.POST.get('driver_night_price_10_15_km_non_ac', '0') or '0')
        driver_night_price_above_15_km_non_ac = Decimal(request.POST.get('driver_night_price_above_15_km_non_ac', '0') or '0')

        # Day Time Ride AC
        customer_day_base_fare_ac = Decimal(request.POST.get('customer_day_base_fare_ac', '0') or '0')
        customer_day_per_min_charge_ac = Decimal(request.POST.get('customer_day_per_min_charge_ac', '0') or '0')
        customer_day_price_0_5_km_ac = Decimal(request.POST.get('customer_day_price_0_5_km_ac', '0') or '0')
        customer_day_price_5_10_km_ac = Decimal(request.POST.get('customer_day_price_5_10_km_ac', '0') or '0')
        customer_day_price_10_15_km_ac = Decimal(request.POST.get('customer_day_price_10_15_km_ac', '0') or '0')
        customer_day_price_above_15_km_ac = Decimal(request.POST.get('customer_day_price_above_15_km_ac', '0') or '0')

        driver_day_base_fare_ac = Decimal(request.POST.get('driver_day_base_fare_ac', '0') or '0')
        driver_day_per_min_charge_ac = Decimal(request.POST.get('driver_day_per_min_charge_ac', '0') or '0')
        driver_day_price_0_5_km_ac = Decimal(request.POST.get('driver_day_price_0_5_km_ac', '0') or '0')
        driver_day_price_5_10_km_ac = Decimal(request.POST.get('driver_day_price_5_10_km_ac', '0') or '0')
        driver_day_price_10_15_km_ac = Decimal(request.POST.get('driver_day_price_10_15_km_ac', '0') or '0')
        driver_day_price_above_15_km_ac = Decimal(request.POST.get('driver_day_price_above_15_km_ac', '0') or '0')

        # Night Time Ride AC
        customer_night_base_fare_ac = Decimal(request.POST.get('customer_night_base_fare_ac', '0') or '0')
        customer_night_per_min_charge_ac = Decimal(request.POST.get('customer_night_per_min_charge_ac', '0') or '0')
        customer_night_price_0_5_km_ac = Decimal(request.POST.get('customer_night_price_0_5_km_ac', '0') or '0')
        customer_night_price_5_10_km_ac = Decimal(request.POST.get('customer_night_price_5_10_km_ac', '0.0') or '0')
        customer_night_price_10_15_km_ac = Decimal(request.POST.get('customer_night_price_10_15_km_ac', '0') or '0')
        customer_night_price_above_15_km_ac = Decimal(request.POST.get('customer_night_price_above_15_km_ac', '0') or '0')

        driver_night_base_fare_ac = Decimal(request.POST.get('driver_night_base_fare_ac', '0') or '0')
        driver_night_per_min_charge_ac = Decimal(request.POST.get('driver_night_per_min_charge_ac', '0') or '0')
        driver_night_price_0_5_km_ac = Decimal(request.POST.get('driver_night_price_0_5_km_ac', '0') or '0')
        driver_night_price_5_10_km_ac = Decimal(request.POST.get('driver_night_price_5_10_km_ac', '0') or '0')
        driver_night_price_10_15_km_ac = Decimal(request.POST.get('driver_night_price_10_15_km_ac', '0') or '0')
        driver_night_price_above_15_km_ac = Decimal(request.POST.get('driver_night_price_above_15_km_ac', '0') or '0')
        ...
        ride_pricing = RidePricing(
            ...
            company_commission=company_commission,
            driver_commission_percentage=driver_commission_percentage,
            vat_tax_percentage=vat_tax_percentage,
            waiting_time_per_min_charge=waiting_time_per_min_charge,
            driver_per_km_commission_type=driver_per_km_commission_type,
            driver_per_km_commission_charge=driver_per_km_commission_charge,
            dead_per_km_charge=dead_per_km_charge,
            customer_day_base_fare_non_ac=customer_day_base_fare_non_ac,
            customer_day_per_min_charge_non_ac=customer_day_per_min_charge_non_ac,
            customer_day_price_0_5_km_non_ac=customer_day_price_0_5_km_non_ac,
            customer_day_price_5_10_km_non_ac=customer_day_price_5_10_km_non_ac,
            customer_day_price_10_15_km_non_ac=customer_day_price_10_15_km_non_ac,
            customer_day_price_above_15_km_non_ac=customer_day_price_above_15_km_non_ac,
            driver_day_base_fare_non_ac=driver_day_base_fare_non_ac,
            driver_day_per_min_charge_non_ac=driver_day_per_min_charge_non_ac,
            driver_day_price_0_5_km_non_ac=driver_day_price_0_5_km_non_ac,
            driver_day_price_5_10_km_non_ac=driver_day_price_5_10_km_non_ac,
            driver_day_price_10_15_km_non_ac=driver_day_price_10_15_km_non_ac,
            driver_day_price_above_15_km_non_ac=driver_day_price_above_15_km_non_ac,
            customer_night_base_fare_non_ac=customer_night_base_fare_non_ac,
            customer_night_per_min_charge_non_ac=customer_night_per_min_charge_non_ac,
            customer_night_price_0_5_km_non_ac=customer_night_price_0_5_km_non_ac,
            customer_night_price_5_10_km_non_ac=customer_night_price_5_10_km_non_ac,
            customer_night_price_10_15_km_non_ac=customer_night_price_10_15_km_non_ac,
            customer_night_price_above_15_km_non_ac=customer_night_price_above_15_km_non_ac,
            driver_night_base_fare_non_ac=driver_night_base_fare_non_ac,
            driver_night_per_min_charge_non_ac=driver_night_per_min_charge_non_ac,
            driver_night_price_0_5_km_non_ac=driver_night_price_0_5_km_non_ac,
            driver_night_price_5_10_km_non_ac=driver_night_price_5_10_km_non_ac,
            driver_night_price_10_15_km_non_ac=driver_night_price_10_15_km_non_ac,
            driver_night_price_above_15_km_non_ac=driver_night_price_above_15_km_non_ac,
            customer_day_base_fare_ac=customer_day_base_fare_ac,
            customer_day_per_min_charge_ac=customer_day_per_min_charge_ac,
            customer_day_price_0_5_km_ac=customer_day_price_0_5_km_ac,
            customer_day_price_5_10_km_ac=customer_day_price_5_10_km_ac,
            customer_day_price_10_15_km_ac=customer_day_price_10_15_km_ac,
            customer_day_price_above_15_km_ac=customer_day_price_above_15_km_ac,
            driver_day_base_fare_ac=driver_day_base_fare_ac,
            driver_day_per_min_charge_ac=driver_day_per_min_charge_ac,
            driver_day_price_0_5_km_ac=driver_day_price_0_5_km_ac,
            driver_day_price_5_10_km_ac=driver_day_price_5_10_km_ac,
            driver_day_price_10_15_km_ac=driver_day_price_10_15_km_ac,
            driver_day_price_above_15_km_ac=driver_day_price_above_15_km_ac,
            customer_night_base_fare_ac=customer_night_base_fare_ac,
            customer_night_per_min_charge_ac=customer_night_per_min_charge_ac,
            customer_night_price_0_5_km_ac=customer_night_price_0_5_km_ac,
            customer_night_price_5_10_km_ac=customer_night_price_5_10_km_ac,
            customer_night_price_10_15_km_ac=customer_night_price_10_15_km_ac,
            customer_night_price_above_15_km_ac=customer_night_price_above_15_km_ac,
            driver_night_base_fare_ac=driver_night_base_fare_ac,
            driver_night_per_min_charge_ac=driver_night_per_min_charge_ac,
            driver_night_price_0_5_km_ac=driver_night_price_0_5_km_ac,
            driver_night_price_5_10_km_ac=driver_night_price_5_10_km_ac,
            driver_night_price_10_15_km_ac=driver_night_price_10_15_km_ac,
            driver_night_price_above_15_km_ac=driver_night_price_above_15_km_ac,
        )

        ride_pricing.save()

        return JsonResponse({'status': 'success', 'message': 'Ride pricing added successfully.'})
    
@method_decorator(login_required(login_url='login'), name='dispatch')
class LocalPriceList(ListView):
    model = RidePricing
    template_name = "superadmin/view_localprice.html"

    # def get_queryset(self):
    #     return RidePricing.objects.filter(price_type='localprice') 

@method_decorator(login_required(login_url='login'), name='dispatch')
class DeleteLocalPrice(View):
    def get(self, request):
        ride_pricing_id = request.GET.get('ride_pricing_id', None)
        RidePricing.objects.get(ride_pricing_id=ride_pricing_id).delete()
        data = {
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class EditLocalPrice(TemplateView):
    template_name = 'superadmin/edit_localprice.html'
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class UpdateLocalPrice(APIView):
    def post(self, request):
        # Update pricing data
        ride_pricing_id = request.POST.get('ride_pricing_id')
        city = request.POST.get('city')
        ...
        company_commission = Decimal(request.POST.get('company_commission'))
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage'))
        vat_tax_percentage = Decimal(request.POST.get('vat_tax_percentage'))
        waiting_time_per_min_charge = Decimal(request.POST.get('waiting_time_per_min_charge'))
        driver_per_km_commission_type = request.POST.get('driver_per_km_commission_type')
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge'))
        driver_per_km_commission_charge = Decimal(request.POST.get('driver_per_km_commission_charge'))

        # Day Time Ride Non AC
        customer_day_base_fare_non_ac = Decimal(request.POST.get('customer_day_base_fare_non_ac'))
        customer_day_per_min_charge_non_ac = Decimal(request.POST.get('customer_day_per_min_charge_non_ac'))
        customer_day_price_0_5_km_non_ac = Decimal(request.POST.get('customer_day_price_0_5_km_non_ac'))
        customer_day_price_5_10_km_non_ac = Decimal(request.POST.get('customer_day_price_5_10_km_non_ac'))
        customer_day_price_10_15_km_non_ac = Decimal(request.POST.get('customer_day_price_10_15_km_non_ac'))
        customer_day_price_above_15_km_non_ac = Decimal(request.POST.get('customer_day_price_above_15_km_non_ac'))
        
        driver_day_base_fare_non_ac = Decimal(request.POST.get('driver_day_base_fare_non_ac'))
        driver_day_per_min_charge_non_ac = Decimal(request.POST.get('driver_day_per_min_charge_non_ac'))
        driver_day_price_0_5_km_non_ac = Decimal(request.POST.get('driver_day_price_0_5_km_non_ac'))
        driver_day_price_5_10_km_non_ac = Decimal(request.POST.get('driver_day_price_5_10_km_non_ac'))
        driver_day_price_10_15_km_non_ac = Decimal(request.POST.get('driver_day_price_10_15_km_non_ac'))
        driver_day_price_above_15_km_non_ac = Decimal(request.POST.get('driver_day_price_above_15_km_non_ac'))

        # Night Time Ride Non AC
        customer_night_base_fare_non_ac = Decimal(request.POST.get('customer_night_base_fare_non_ac'))
        customer_night_per_min_charge_non_ac = Decimal(request.POST.get('customer_night_per_min_charge_non_ac'))
        customer_night_price_0_5_km_non_ac = Decimal(request.POST.get('customer_night_price_0_5_km_non_ac'))
        customer_night_price_5_10_km_non_ac = Decimal(request.POST.get('customer_night_price_5_10_km_non_ac'))
        customer_night_price_10_15_km_non_ac = Decimal(request.POST.get('customer_night_price_10_15_km_non_ac'))
        customer_night_price_above_15_km_non_ac = Decimal(request.POST.get('customer_night_price_above_15_km_non_ac'))

        driver_night_base_fare_non_ac = Decimal(request.POST.get('driver_night_base_fare_non_ac'))
        driver_night_per_min_charge_non_ac = Decimal(request.POST.get('driver_night_per_min_charge_non_ac'))
        driver_night_price_0_5_km_non_ac = Decimal(request.POST.get('driver_night_price_0_5_km_non_ac'))
        driver_night_price_5_10_km_non_ac = Decimal(request.POST.get('driver_night_price_5_10_km_non_ac'))
        driver_night_price_10_15_km_non_ac = Decimal(request.POST.get('driver_night_price_10_15_km_non_ac'))
        driver_night_price_above_15_km_non_ac = Decimal(request.POST.get('driver_night_price_above_15_km_non_ac'))

        # Day Time Ride AC
        customer_day_base_fare_ac = Decimal(request.POST.get('customer_day_base_fare_ac'))
        customer_day_per_min_charge_ac = Decimal(request.POST.get('customer_day_per_min_charge_ac'))
        customer_day_price_0_5_km_ac = Decimal(request.POST.get('customer_day_price_0_5_km_ac'))
        customer_day_price_5_10_km_ac = Decimal(request.POST.get('customer_day_price_5_10_km_ac'))
        customer_day_price_10_15_km_ac = Decimal(request.POST.get('customer_day_price_10_15_km_ac'))
        customer_day_price_above_15_km_ac = Decimal(request.POST.get('customer_day_price_above_15_km_ac'))

        driver_day_base_fare_ac = Decimal(request.POST.get('driver_day_base_fare_ac'))
        driver_day_per_min_charge_ac = Decimal(request.POST.get('driver_day_per_min_charge_ac'))
        driver_day_price_0_5_km_ac = Decimal(request.POST.get('driver_day_price_0_5_km_ac'))
        driver_day_price_5_10_km_ac = Decimal(request.POST.get('driver_day_price_5_10_km_ac'))
        driver_day_price_10_15_km_ac = Decimal(request.POST.get('driver_day_price_10_15_km_ac'))
        driver_day_price_above_15_km_ac = Decimal(request.POST.get('driver_day_price_above_15_km_ac'))

        # Night Time Ride AC
        customer_night_base_fare_ac = Decimal(request.POST.get('customer_night_base_fare_ac'))
        customer_night_per_min_charge_ac = Decimal(request.POST.get('customer_night_per_min_charge_ac'))
        customer_night_price_0_5_km_ac = Decimal(request.POST.get('customer_night_price_0_5_km_ac'))
        customer_night_price_5_10_km_ac = Decimal(request.POST.get('customer_night_price_5_10_km_ac'))
        customer_night_price_10_15_km_ac = Decimal(request.POST.get('customer_night_price_10_15_km_ac'))
        customer_night_price_above_15_km_ac = Decimal(request.POST.get('customer_night_price_above_15_km_ac'))

        driver_night_base_fare_ac = Decimal(request.POST.get('driver_night_base_fare_ac'))
        driver_night_per_min_charge_ac = Decimal(request.POST.get('driver_night_per_min_charge_ac'))
        driver_night_price_0_5_km_ac = Decimal(request.POST.get('driver_night_price_0_5_km_ac'))
        driver_night_price_5_10_km_ac = Decimal(request.POST.get('driver_night_price_5_10_km_ac'))
        driver_night_price_10_15_km_ac = Decimal(request.POST.get('driver_night_price_10_15_km_ac'))
        driver_night_price_above_15_km_ac = Decimal(request.POST.get('driver_night_price_above_15_km_ac'))



        # Create a CustomerHistory entry after updating the customer
        local = RidePricing.objects.get(ride_pricing_id=ride_pricing_id)
        local.ride_pricing_id=ride_pricing_id,
        local.city=city,
        local.car_category=car_category,
        local.company_commission=company_commission,
        local.driver_commission_percentage=driver_commission_percentage,
        local.vat_tax_percentage=vat_tax_percentage,
        local.waiting_time_per_min_charge=waiting_time_per_min_charge,
        local.driver_per_km_commission_type=driver_per_km_commission_type,
        local.dead_per_km_charge=dead_per_km_charge,
        local.driver_per_km_commission_charge=driver_per_km_commission_charge,
        local.customer_day_base_fare_non_ac=customer_day_base_fare_non_ac,
        local.customer_day_per_min_charge_non_ac=customer_day_per_min_charge_non_ac,
        local.customer_day_price_0_5_km_non_ac=customer_day_price_0_5_km_non_ac,
        local.customer_day_price_5_10_km_non_ac=customer_day_price_5_10_km_non_ac,
        local.customer_day_price_10_15_km_non_ac=customer_day_price_10_15_km_non_ac,
        local.customer_day_price_above_15_km_non_ac=customer_day_price_above_15_km_non_ac,
        local.driver_day_base_fare_non_ac=driver_day_base_fare_non_ac,
        local.driver_day_per_min_charge_non_ac=driver_day_per_min_charge_non_ac,
        local.driver_day_price_0_5_km_non_ac=driver_day_price_0_5_km_non_ac,
        local.driver_day_price_5_10_km_non_ac=driver_day_price_5_10_km_non_ac,
        local.driver_day_price_10_15_km_non_ac=driver_day_price_10_15_km_non_ac,
        local.driver_day_price_above_15_km_non_ac=driver_day_price_above_15_km_non_ac,
        local.customer_night_base_fare_non_ac=customer_night_base_fare_non_ac,
        local.customer_night_per_min_charge_non_ac=customer_night_per_min_charge_non_ac,
        local.customer_night_price_0_5_km_non_ac=customer_night_price_0_5_km_non_ac,
        local.customer_night_price_5_10_km_non_ac=customer_night_price_5_10_km_non_ac,
        local.customer_night_price_10_15_km_non_ac=customer_night_price_10_15_km_non_ac,
        local.customer_night_price_above_15_km_non_ac=customer_night_price_above_15_km_non_ac,
        local.driver_night_base_fare_non_ac=driver_night_base_fare_non_ac,
        local.driver_night_per_min_charge_non_ac=driver_night_per_min_charge_non_ac,
        local.driver_night_price_0_5_km_non_ac=driver_night_price_0_5_km_non_ac,
        local.driver_night_price_5_10_km_non_ac=driver_night_price_5_10_km_non_ac,
        local.driver_night_price_10_15_km_non_ac=driver_night_price_10_15_km_non_ac,
        local.driver_night_price_above_15_km_non_ac=driver_night_price_above_15_km_non_ac,
        local.customer_day_base_fare_ac=customer_day_base_fare_ac,
        local.customer_day_per_min_charge_ac=customer_day_per_min_charge_ac,
        local.customer_day_price_0_5_km_ac=customer_day_price_0_5_km_ac,
        local.customer_day_price_5_10_km_ac=customer_day_price_5_10_km_ac,
        local.customer_day_price_10_15_km_ac=customer_day_price_10_15_km_ac,
        local.customer_day_price_above_15_km_ac=customer_day_price_above_15_km_ac,
        local.driver_day_base_fare_ac=driver_day_base_fare_ac,
        local.driver_day_per_min_charge_ac=driver_day_per_min_charge_ac,
        local.driver_day_price_0_5_km_ac=driver_day_price_0_5_km_ac,
        local.driver_day_price_5_10_km_ac=driver_day_price_5_10_km_ac,
        local.driver_day_price_10_15_km_ac=driver_day_price_10_15_km_ac,
        local.driver_day_price_above_15_km_ac=driver_day_price_above_15_km_ac,
        local.customer_night_base_fare_ac=customer_night_base_fare_ac,
        local.customer_night_per_min_charge_ac=customer_night_per_min_charge_ac,
        local.customer_night_price_0_5_km_ac=customer_night_price_0_5_km_ac,
        local.customer_night_price_5_10_km_ac=customer_night_price_5_10_km_ac,
        local.customer_night_price_10_15_km_ac=customer_night_price_10_15_km_ac,
        local.customer_night_price_above_15_km_ac=customer_night_price_above_15_km_ac,
        local.driver_night_base_fare_ac=driver_night_base_fare_ac,
        local.driver_night_per_min_charge_ac=driver_night_per_min_charge_ac,
        local.driver_night_price_0_5_km_ac=driver_night_price_0_5_km_ac,
        local.driver_night_price_5_10_km_ac=driver_night_price_5_10_km_ac,
        local.driver_night_price_10_15_km_ac=driver_night_price_10_15_km_ac,
        local.driver_night_price_above_15_km_ac=driver_night_price_above_15_km_ac,
        local.save()

        return JsonResponse({'success': True}, status=200)        

@method_decorator(login_required(login_url='login'), name='dispatch')
class AddAirportPricingView(TemplateView):
    template_name = "superadmin/add_airportprice.html"
    ...
    def post(self, request):
        ...
        company_commission = Decimal(request.POST.get('company_commission', '0') or '0')
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage', '0') or '0')
        customer_min_amount_ac = Decimal(request.POST.get('customer_min_amount_ac', '0') or '0')
        driver_min_amount_ac = Decimal(request.POST.get('driver_min_amount_ac', '0') or '0')
        customer_min_amount_non_ac = Decimal(request.POST.get('customer_min_amount_non_ac', '0') or '0')
        driver_min_amount_non_ac = Decimal(request.POST.get('driver_min_amount_non_ac', '0') or '0')
        customer_min_km_day_ac = Decimal(request.POST.get('customer_min_km_day_ac', '0') or '0')
        driver_min_km_day_ac = Decimal(request.POST.get('driver_min_km_day_ac', '0') or '0')
        customer_min_km_day_non_ac = Decimal(request.POST.get('customer_min_km_day_non_ac', '0') or '0')
        driver_min_km_day_non_ac = Decimal(request.POST.get('driver_min_km_day_non_ac', '0') or '0')
        customer_extra_hour_km_charge_ac = Decimal(request.POST.get('customer_extra_hour_km_charge_ac', '0') or '0')
        driver_extra_hour_km_charge_ac = Decimal(request.POST.get('driver_extra_hour_km_charge_ac', '0') or '0')
        customer_extra_hour_km_charge_non_ac = Decimal(request.POST.get('customer_extra_hour_km_charge_non_ac', '0') or '0')
        driver_extra_hour_km_charge_non_ac = Decimal(request.POST.get('driver_extra_hour_km_charge_non_ac', '0') or '0')
        customer_extra_km_charge_ac = Decimal(request.POST.get('customer_extra_km_charge_ac', '0') or '0')
        driver_extra_km_charge_ac = Decimal(request.POST.get('driver_extra_km_charge_ac', '0') or '0')
        customer_extra_km_charge_non_ac = Decimal(request.POST.get('customer_extra_km_charge_non_ac', '0') or '0')
        driver_extra_km_charge_non_ac = Decimal(request.POST.get('driver_extra_km_charge_non_ac', '0') or '0')
        gst_tax_percentage = Decimal(request.POST.get('gst_tax_percentage', '0') or '0')
        additional_info = request.POST['additional_info']
        driver_per_km_commission_type = request.POST['driver_per_km_commission_type']
        driver_per_km_commission_charge = Decimal(request.POST.get('driver_per_km_commission_charge', '0') or '0')
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge', '0') or '0')
        toll_charge = Decimal(request.POST.get('toll_charge', '0') or '0')

        # Create and save AirportPricing instance
        airport_pricing = RidePricing(
            city=city,
            trip_type=trip_type,
            price_type=price_type,
            car_category=Model.objects.get(model_id=model_id),
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class AirportPriceList(ListView):
    model = RidePricing
    template_name = "superadmin/view_airportprice.html"

@method_decorator(login_required(login_url='login'), name='dispatch')
class DeleteAirportPrice(View):
    def  get(self, request):
        ride_pricing_id = request.GET.get('ride_pricing_id', None)
        RidePricing.objects.get(ride_pricing_id=ride_pricing_id).delete()
        data = {
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class EditAirportPackage(TemplateView):
    template_name = 'superadmin/edit_airportprice.html'
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class UpdateAirportPackage(APIView):
    def post(self, request):
        # Update airport package data
        ride_pricing_id = request.POST.get('ride_pricing_id')
        ...
        company_commission = Decimal(request.POST.get('company_commission'))
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage'))
        customer_min_amount_ac = Decimal(request.POST.get('customer_min_amount_ac'))
        driver_min_amount_ac = Decimal(request.POST.get('driver_min_amount_ac'))
        customer_min_amount_non_ac = Decimal(request.POST.get('customer_min_amount_non_ac'))
        driver_min_amount_non_ac = Decimal(request.POST.get('driver_min_amount_non_ac'))
        customer_min_km_day_ac = Decimal(request.POST.get('customer_min_km_day_ac'))
        driver_min_km_day_ac = Decimal(request.POST.get('driver_min_km_day_ac'))
        customer_min_km_day_non_ac = Decimal(request.POST.get('customer_min_km_day_non_ac'))
        driver_min_km_day_non_ac = Decimal(request.POST.get('driver_min_km_day_non_ac'))
        customer_extra_hour_km_charge_ac = Decimal(request.POST.get('customer_extra_hour_km_charge_ac'))
        driver_extra_hour_km_charge_ac = Decimal(request.POST.get('driver_extra_hour_km_charge_ac'))
        customer_extra_hour_km_charge_non_ac = Decimal(request.POST.get('customer_extra_hour_km_charge_non_ac'))
        driver_extra_hour_km_charge_non_ac = Decimal(request.POST.get('driver_extra_hour_km_charge_non_ac'))
        customer_extra_km_charge_ac = Decimal(request.POST.get('customer_extra_km_charge_ac'))
        driver_extra_km_charge_ac = Decimal(request.POST.get('driver_extra_km_charge_ac'))
        customer_extra_km_charge_non_ac = Decimal(request.POST.get('customer_extra_km_charge_non_ac'))
        driver_extra_km_charge_non_ac = Decimal(request.POST.get('driver_extra_km_charge_non_ac'))
        gst_tax_percentage = Decimal(request.POST.get('gst_tax_percentage'))
        additional_info = request.POST.get('additional_info')
        driver_per_km_commission_type = request.POST.get('driver_per_km_commission_type')
        driver_per_km_commission_charge = Decimal(request.POST.get('driver_per_km_commission_charge'))
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge'))
        toll_charge = Decimal(request.POST.get('toll_charge'))

        # Save updated fields

        # Create a RidePricingHistory entry after updating
        local = RidePricing.objects.get(ride_pricing_id=ride_pricing_id)
        local.ride_pricing_id=ride_pricing_id,
        local.price_type=price_type,
        local.city=city,
        local.trip_type=trip_type,
        local.car_category=car_category,
        local.company_commission=company_commission,
        local.driver_commission_percentage=driver_commission_percentage,
        local.customer_min_amount_ac=customer_min_amount_ac,
        local.driver_min_amount_ac=driver_min_amount_ac,
        local.customer_min_amount_non_ac=customer_min_amount_non_ac,
        local.driver_min_amount_non_ac=driver_min_amount_non_ac,
        local.customer_min_km_day_ac=customer_min_km_day_ac,
        local.driver_min_km_day_ac=driver_min_km_day_ac,
        local.customer_min_km_day_non_ac=customer_min_km_day_non_ac,
        local.driver_min_km_day_non_ac=driver_min_km_day_non_ac,
        local.customer_extra_hour_km_charge_ac=customer_extra_hour_km_charge_ac,
        local.driver_extra_hour_km_charge_ac=driver_extra_hour_km_charge_ac,
        local.customer_extra_hour_km_charge_non_ac=customer_extra_hour_km_charge_non_ac,
        local.driver_extra_hour_km_charge_non_ac=driver_extra_hour_km_charge_non_ac,
        local.customer_extra_km_charge_ac=customer_extra_km_charge_ac,
        local.driver_extra_km_charge_ac=driver_extra_km_charge_ac,
        local.customer_extra_km_charge_non_ac=customer_extra_km_charge_non_ac,
        local.driver_extra_km_charge_non_ac=driver_extra_km_charge_non_ac,
        local.gst_tax_percentage=gst_tax_percentage,
        local.additional_info=additional_info,
        local.driver_per_km_commission_type=driver_per_km_commission_type,
        local.driver_per_km_commission_charge=driver_per_km_commission_charge,
        local.dead_per_km_charge=dead_per_km_charge,
        local.toll_charge=toll_charge,
        local.save()

        return JsonResponse({'success': True}, status=200)        

@method_decorator(login_required(login_url='login'), name='dispatch')
class AddOutstationView(TemplateView):
    template_name = "superadmin/add_outstationprice.html"
    ...
    def post(self, request):
        ...
        customer_min_amount_non_ac = Decimal(request.POST.get('customer_min_amount_non_ac', '0') or '0')
        driver_min_amount_non_ac = Decimal(request.POST.get('driver_min_amount_non_ac', '0') or '0')
        customer_min_km_per_day_ac = Decimal(request.POST.get('customer_min_km_per_day_ac', '0') or '0')
        driver_max_km_per_day_ac = Decimal(request.POST.get('driver_max_km_per_day_ac', '0') or '0')
        customer_min_km_per_day_non_ac = Decimal(request.POST.get('customer_min_km_per_day_non_ac', '0') or '0')
        driver_max_km_per_day_non_ac = Decimal(request.POST.get('driver_max_km_per_day_non_ac', '0') or '0')
        customer_extra_km_charge_ac = Decimal(request.POST.get('customer_extra_km_charge_ac', '0') or '0')
        driver_extra_km_charge_ac = Decimal(request.POST.get('driver_extra_km_charge_ac', '0') or '0')
        customer_extra_km_charge_non_ac = Decimal(request.POST.get('customer_extra_km_charge_non_ac', '0') or '0')
        driver_extra_km_charge_non_ac = Decimal(request.POST.get('driver_extra_km_charge_non_ac', '0') or '0')
        ...
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge', '0') or '0')
        driver_allowance = Decimal(request.POST.get('driver_allowance', '0') or '0')
        driver_night_bata = Decimal(request.POST.get('driver_night_bata', '0') or '0')

        outstation_data = RidePricing(
            price_type=price_type,
            city=city,
            trip_type=trip_type,
            car_category=Model.objects.get(model_id=model_id),
            ...
            customer_min_amount_non_ac=customer_min_amount_non_ac,
            driver_min_amount_non_ac=driver_min_amount_non_ac,
            customer_min_km_per_day_ac=customer_min_km_per_day_ac,
            driver_max_km_per_day_ac=driver_max_km_per_day_ac,
            customer_min_km_per_day_non_ac=customer_min_km_per_day_non_ac,
            driver_max_km_per_day_non_ac=driver_max_km_per_day_non_ac,
            customer_extra_km_charge_ac=customer_extra_km_charge_ac,
            driver_extra_km_charge_ac=driver_extra_km_charge_ac,
            customer_extra_km_charge_non_ac=customer_extra_km_charge_non_ac,
            driver_extra_km_charge_non_ac=driver_extra_km_charge_non_ac,
            ...
            dead_per_km_charge=dead_per_km_charge,
            driver_allowance=driver_allowance,
            driver_night_bata=driver_night_bata,
        )
        outstation_data.save()
        return JsonResponse({'status': "Success"})
    
@method_decorator(login_required(login_url='login'), name='dispatch')
class OutstationPriceList(ListView):
    model = RidePricing
    template_name = "superadmin/view_outstationprice.html"

@method_decorator(login_required(login_url='login'), name='dispatch')
class DeletestationPrice(View):
    def  get(self, request):
        ride_pricing_id = request.GET.get('ride_pricing_id', None)
        RidePricing.objects.get(ride_pricing_id=ride_pricing_id).delete()
        data = {
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class EditOutstationPrice(TemplateView):
    template_name = 'superadmin/edit_outstationprice.html'
...
@method_decorator(login_required(login_url='login'), name='dispatch')
class UpdateOutstationPrice(APIView):
    def post(self, request):
        # Update pricing data
        ride_pricing_id = request.POST.get('ride_pricing_id')
        ...
        city = request.POST.get('city')
        trip_type = request.POST.get('trip_type')
        model_id =request.POST.get('car_category')
        company_commission = Decimal(request.POST.get('company_commission'))
        driver_commission_percentage = Decimal(request.POST.get('driver_commission_percentage'))
        customer_min_amount_ac = Decimal(request.POST.get('customer_min_amount_ac'))
        driver_min_amount_ac = Decimal(request.POST.get('driver_min_amount_ac'))
        customer_min_amount_non_ac = Decimal(request.POST.get('customer_min_amount_non_ac'))
        driver_min_amount_non_ac = Decimal(request.POST.get('driver_min_amount_non_ac'))
        customer_min_km_per_day_ac = Decimal(request.POST.get('customer_min_km_per_day_ac'))
        driver_max_km_per_day_ac = Decimal(request.POST.get('driver_max_km_per_day_ac'))
        customer_min_km_per_day_non_ac = Decimal(request.POST.get('customer_min_km_per_day_non_ac'))
        driver_max_km_per_day_non_ac = Decimal(request.POST.get('driver_max_km_per_day_non_ac'))
        customer_extra_km_charge_ac = Decimal(request.POST.get('customer_extra_km_charge_ac'))
        driver_extra_km_charge_ac = Decimal(request.POST.get('driver_extra_km_charge_ac'))
        customer_extra_km_charge_non_ac = Decimal(request.POST.get('customer_extra_km_charge_non_ac'))
        driver_extra_km_charge_non_ac = Decimal(request.POST.get('driver_extra_km_charge_non_ac'))
        ...
        dead_per_km_charge = Decimal(request.POST.get('dead_per_km_charge'))
        driver_allowance = Decimal(request.POST.get('driver_allowance'))
        driver_night_bata = Decimal(request.POST.get('driver_night_bata'))
        # Save updated fields
        ...
        local.customer_min_amount_non_ac=customer_min_amount_non_ac,
        local.driver_min_amount_non_ac=driver_min_amount_non_ac,
        local.customer_min_km_per_day_ac=customer_min_km_per_day_ac,
        local.driver_max_km_per_day_ac=driver_max_km_per_day_ac,
        local.customer_min_km_per_day_non_ac=customer_min_km_per_day_non_ac,
        local.driver_max_km_per_day_non_ac=driver_max_km_per_day_non_ac,
        local.customer_extra_km_charge_ac=customer_extra_km_charge_ac,
        local.driver_extra_km_charge_ac=driver_extra_km_charge_ac,
        local.customer_extra_km_charge_non_ac=customer_extra_km_charge_non_ac,
        local.driver_extra_km_charge_non_ac=driver_extra_km_charge_non_ac,
        ...
        local.dead_per_km_charge=dead_per_km_charge,
        local.driver_allowance=driver_allowance,
        local.driver_night_bata=driver_night_bata,
        local.save()
